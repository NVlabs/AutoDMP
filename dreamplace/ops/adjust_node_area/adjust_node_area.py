...
class ComputeNodeAreaFromRouteMap(nn.Module):
    def __init__(self, xl, yl, xh, yh, num_movable_nodes, num_bins_x, num_bins_y):
...
class ComputeNodeAreaFromPinMap(ComputeNodeAreaFromRouteMap):
    def __init__(
        ...
        super(ComputeNodeAreaFromPinMap, self).__init__(
            xl, yl, xh, yh, num_movable_nodes, num_bins_x, num_bins_y
        )
        ...
        if pin_weights is not None:
            ...
            self.pin_weights = (
                flat_node2pin_start_map[1 : self.num_movable_nodes + 1]
                - flat_node2pin_start_map[: self.num_movable_nodes]
            )
        else:
...
class AdjustNodeArea(nn.Module):
    def __init__(
        ...
        self.compute_node_area_route = ComputeNodeAreaFromRouteMap(
            xl=self.fp_info.routing_grid_xl,
            yl=self.fp_info.routing_grid_yl,
            xh=self.fp_info.routing_grid_xh,
            yh=self.fp_info.routing_grid_yh,
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            scale_factor = (
                min(
                    0.1 * self.total_whitespace_area,
            ...
            if adjust_route_area_flag:
                route_area_increment_ratio = (
                    F.relu(route_opt_area - old_movable_area).sum()
                    / old_movable_area_sum
                )
                adjust_route_area_flag = (
                    route_area_increment_ratio.data.item()
            ...
            if adjust_pin_area_flag:
                pin_area_increment_ratio = (
                    F.relu(pin_opt_area - old_movable_area).sum() / old_movable_area_sum
                )
                adjust_pin_area_flag = (
                    pin_area_increment_ratio.data.item()
            ...
            pos.data[num_nodes : num_nodes + self.num_movable_nodes] += (
                node_size_y_movable * 0.5
            )
            # scale size
            ...
            if new_movable_area_sum + old_filler_area_sum > self.total_place_area:
                new_filler_area_sum = F.relu(
                    self.total_place_area - new_movable_area_sum
                )
                filler_nodes_ratio = new_filler_area_sum / old_filler_area_sum
                logger.info(
                    "inflation ratio for filler nodes: %g" % (filler_nodes_ratio)
                )
                filler_nodes_ratio.sqrt_()
                # convert positions to centers
                pos.data[num_nodes - self.num_filler_nodes : num_nodes] += (
                    node_size_x_filler * 0.5
                )
            ...
            target_density.data.copy_(
                (new_movable_area_sum + new_filler_area_sum) / self.total_place_area
            )
