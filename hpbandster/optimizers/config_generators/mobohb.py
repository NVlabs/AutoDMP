...
class MOBOHB(base_config_generator):
    def __init__(self, configspace, parameters, history_dir, run_id, init=True, min_points_in_model=None,
                 top_n_percent=10, num_samples=24, random_fraction=1 / 6,
    ...
    def get_config(self, budget):
        ...
        if self.mobohb_with_init:
            ...
            if np.random.rand() < self.random_fraction or budget not in self.configs or not np.any(budget_counts > self.min_points_in_model):
                sample = self.configspace.sample_configuration()
                info_dict['model_based_pick'] = False
        ...
        if sample is None:
            try:
                ...
                history_largest_budget = [h for h in self.history if h['budget'] == largest_budget]
