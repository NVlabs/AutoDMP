...
class BaseIteration(object):
	...
	def register_result(self, job, skip_sanity_checks=False):
		...
		if (not job.result is None) and np.all(np.isfinite(result['loss'])):
			d.status = 'REVIEW'
		else:
			d.status = 'CRASHED'
	...
	def process_results(self):
		...
		for i, cid in enumerate(config_ids):
			if advance[i]:
				...
				config = self.data[cid].config
				config_hash = int(hashlib.sha256(str(config).encode('utf-8')).hexdigest(), 16) % 10 ** 10
				for j in range(3):
					if os.path.exists(os.path.join('tmp', str(int(np.ceil(budget))), str(config_hash + j) + ".pt")):
						os.remove(os.path.join('tmp', str(int(np.ceil(budget))), str(config_hash + j) + ".pt"))
